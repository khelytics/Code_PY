...
nato_df['expedite'] = np.where(
    nato_df.transcript.str.contains('(?<=unidentified).*?expedite.*?(?=agent)'), '1', '0'
)

nato_df['fedex'] = np.where(
    nato_df.transcript.str.contains('(?<=unidentified).*?fedex.*?(?=agent)'), '1', '0'
)

nato_df['quick'] = np.where(
    nato_df.transcript.str.contains('(?<=unidentified).*?quick.*?(?=agent)'), '1', '0'
)

nato_df['fast'] = np.where(
    nato_df.transcript.str.contains('(?<=unidentified).*?fast.*?(?=agent)'), '1', '0'
)
